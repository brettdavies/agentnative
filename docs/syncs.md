...
flowchart LR
    ...
    spec -->|"manual<br/>sync-spec.sh"| site
...
Solid arrows are wired `repository_dispatch` events OR manual sync-script runs on the standard dispatch list; dashed
arrows are manual-only and not part of the dispatch fan-out. The skill repo is intentionally absent from the dispatch
fan-out (see Release / dispatch chain below); its release flow re-runs `sync-spec.sh` from the release branch instead.

| Consumer                                                                            | Mechanism                                                                                                                                                                                                                                                                      | What's synced                                                                                                                            | Trigger                                                                                                                                                                                                                                              | Drift check                                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`brettdavies/agentnative-cli`](https://github.com/brettdavies/agentnative-cli)     | `scripts/sync-spec.sh` in **cli repo** (manual run); default resolves the latest `v*` tag via `gh api`, falls back to local checkout when offline. `--ref <branch\|tag\|SHA>` (or `SPEC_REF` env) vendors any explicit ref for cross-repo coordination of in-flight spec work. | `principles/p*-*.md` + `VERSION` + `CHANGELOG.md` → `src/principles/spec/`                                                               | On spec release; rerun after every new tag. `build.rs` codegens the `REQUIREMENTS` slice from the vendored copy. Cross-repo coordination: CLI can vendor spec `dev` (or a SHA) before a tag cuts when a CLI change consumes in-flight spec contract. | `build.rs` fails on parse error / missing fields / duplicate IDs. Pre-push hook validates frontmatter parity.                                                                                                                                                                                                                             |
| [`brettdavies/agentnative-skill`](https://github.com/brettdavies/agentnative-skill) | `scripts/sync-spec.sh` in **skill repo** (manual run); same `gh api` resolution, mirrors cli's script. `--ref` / `SPEC_REF` surface identical.                                                                                                                                 | `principles/p*-*.md` + `VERSION` + `CHANGELOG.md` → `spec/`                                                                              | On spec release; ships as part of the skill bundle so consuming agents carry the canonical principle text alongside skill metadata.                                                                                                                  | The vendored tree is checked into the skill bundle; stale orphan files surface via `git status` at commit time per the script's own comment. The skill bundle's release flow re-runs `sync-spec.sh` on the release branch (see skill repo's `RELEASES.md` §"Spec re-vendoring").                                                          |
| [`brettdavies/agentnative-site`](https://github.com/brettdavies/agentnative-site)   | `scripts/sync-spec.sh` in **site repo** (manual run); same `gh api` resolution as cli + skill. `--ref` / `SPEC_REF` surface identical.                                                                                                                                         | `principles/p*-*.md` + `VERSION` + `CHANGELOG.md` → `src/data/spec/`                                                                     | On spec release; rerun after every new tag. The vendored `VERSION` feeds the site footer, OG card, and badge URLs via `src/build/util.mjs:SPEC_VERSION`.                                                                                             | None automated on the site side; `git diff src/data/spec/` after sync is the review surface. Plan that introduced the wiring: `agentnative-site/docs/plans/2026-04-23-001-feat-sync-spec-plan.md`. Cross-repo coordination: site can vendor spec `dev` (or a SHA) to validate site work against in-flight spec contract before a release. |
...
| `agentnative-site`  | `src/data/spec/`                  | `principles/p*-*.md`, `VERSION`, `CHANGELOG.md` |

The cli, skill, and site scripts are intentionally near-identical mirrors; only `DEST_DIR` differs. All three use `gh
api` against the GitHub contents endpoint to vendor files at the resolved ref, with a local-checkout fallback for
offline runs. The `--ref <branch|tag|SHA>` flag (and the matching `SPEC_REF` env var) is uniform across all three.
...
- CLI's implementation: `agentnative-cli/scripts/sync-spec.sh`
- Skill's implementation (mirror of cli's): `agentnative-skill/scripts/sync-spec.sh`
- Site's implementation (mirror of cli's): `agentnative-site/scripts/sync-spec.sh`
